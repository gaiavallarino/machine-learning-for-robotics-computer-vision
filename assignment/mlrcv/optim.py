...
def sgd_momentum(w, dw, config=None):
    ...
    v = config['momentum'] * v - config['learning_rate'] * dw # FIXXXX
    next_w = w + v                                            # FIXXXXXX
...
def rmsprop(w, dw, config=None):
    ...
    keys = ['learning_rate','decay_rate','epsilon','cache'] # FIXXXXX
    lr, dr, eps, cache = (config.get(key) for key in keys)  # FIXXXXXXX

    config['cache'] = dr * cache + (1 - dr) * dw**2         # FIXXXXXX
    next_w = w - lr * dw / (np.sqrt(config['cache']) + eps) # FIXXXXXXXXXXXX
